...
game_on(Desk) ->
	...
	SoList = sj_ai_mod:make_so_data(Seats), 	
	dcfg:rpc_cast_hub(vipm_ctrl_api, vipm_update, [RoomKey, ?one, BoutIdx, SoList]),
	desk_api:broadcast(Desk, sj_desk_api:msg(?A_DESK_SJ_GAME_DATA, {State, BoutIdx, BoutTotal, BankerUid, Tp, Seats})),
...
round_over(Desk) ->
	...
	PointerMsg = sj_desk_api:msg(?A_DESK_SJ_CARDS_EVENT, {Muid, ?alt_pointer, []}),
	DelaysNew = [{fun desk_api:broadcast/2, [self(), PointerMsg], ?two} | Delays],
	Pcurr = sj_ai_mod:get_profit(Seats, Muid, element(?three, lists:unzip3(Elist))),
	Pacc = Opacc + Pcurr,
	SjDNew = SjD#sj_d{profit=Pacc},
	desk_api:broadcast(Desk, sj_desk_api:msg(?A_DESK_SJ_SCORE_DATA, {Pacc, Pcurr})),
...
deal_card(Desk) ->
	...
	{CardsList, CardHeap} = lists:split(?CONST_SJ_GAME_STATE_NUM_PLAYER, sj_ai_api:dealing2()),
	Iter = fun({#seat{uid=Uid,mpid=Mpid,ext_s=SjS}=Seat, Cards}, SeatsNew) ->
				   msg:send(Mpid, sj_desk_api:msg(?A_DESK_SJ_CARDS_EVENT, {Uid, ?dealing, Cards})),
...
lord_card(Desk, Uid, Cards, Behavior) ->
	...
	desk_api:broadcast(Desk, sj_desk_api:msg(?A_DESK_SJ_CARDS_EVENT, {Uid, Behavior, Cards})),
...
bury_card(Desk) ->
	...
	SeatsNew = sj_ai_mod:set_stand_for(Seats, BankerUid),
	msg:send(BankerUid, sj_desk_api:msg(?A_DESK_SJ_CARDS_EVENT, {BankerUid, ?burying, CardHeap})),
	BuryMsg = sj_desk_api:msg(?A_DESK_SJ_CARDS_EVENT, {BankerUid, ?burying, []}),
	GameMsg = sj_desk_api:msg(?A_DESK_SJ_GAME_DATA, {State, BoutIdx, BoutTotal, BankerUid, Tp, SeatsNew}),
...
flip_card(Desk) ->
	...
	Flipper = sj_ai_mod:get_stand_for(Seats, Muid),
	Mps = [sj_ai_api:get_point(Ts, Tp, C) || C <- Mcards],
	Ptotal = sj_ai_api:eval_flip(Flipper, Mps, Heap) + Profit,
	Winner = ?IF(Ptotal >= 80, ?attacker, ?defender),
	NewBankerUid = sj_ai_mod:pick_banker(Winner, BankerUid, Seats),
	{_UpScore, UpLevel, SeatsNew} = sj_ai_mod:up_level(Mode, Seats, Winner, Ptotal, ?one, IsUplevel),
	FlipMsg = sj_desk_api:msg(?A_DESK_SJ_FLIPING, {Flipper, Heap, {Ptotal, Ptotal - Profit}}),
	GameMsg = sj_desk_api:msg(?A_DESK_SJ_GAME_DATA, {State, BoutIdx, BoutTotal, BankerUid, Tp, SeatsNew}),
	SettleMsg = sj_desk_api:msg(?A_DESK_SJ_SETTLEMENT, {Ptotal, Winner, UpLevel, BankerUid, SeatsNew}),
...
eventing(Desk, ?burying, Banker, Cards) ->
	...
	desk_api:broadcast(Desk, sj_desk_api:msg(?A_DESK_SJ_CARDS_EVENT, {BankerUid, ?alt_pointer, []})),
...
eventing(Desk, ?leading, Seat, Cards) ->
	...
	sj_ai_mod:leading(Desk, Seat, Cards);
%%	提示出牌
eventing(Desk, ?prompt, Seat, _Cards) ->
	...
	case Eventing of
		#de{event_list=[{_Raction, _Ruid, Rcards}|_]}	->
			Prompt = sj_ai_mod:prompt(Ts, Tp, Hcards, Rcards),
			msg:send(Mpid, sj_desk_api:msg(?A_DESK_SJ_CARDS_EVENT, {Uid, ?prompt, Prompt})),
			Desk;
		_Otherwise	-> 
			Desk
	end.
